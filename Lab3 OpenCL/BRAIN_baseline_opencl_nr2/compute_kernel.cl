...
void CompDend(global mod_prec *cellCompParamsPtr, int k, int j){
    ...
    global mod_prec *chPrms_v;
    global mod_prec *chPrms_prevComp1, *chPrms_prevComp2;
    global mod_prec *chPrms_newComp1;// *chPrms_newComp2;
    global mod_prec *chComps_iApp;
	global mod_prec *chComps_neighbours;
    mod_prec chComps_iC;
    global mod_prec *chComps_vDend;
	mod_prec vDend;
    global mod_prec *chComps_vSoma;
    global mod_prec *chComps_q, *chComps_r, *chComps_s;
    global mod_prec *chComps_newVDend;
    global mod_prec *chComps_newI_CaH;

    // printf("Dendrite ");

    //Prepare pointers to inputs/outputs
    chPrms_v = &cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + PREVSTATESTARTADD + DEND_V]; //&cellCompParamsPtr->prevCellState->dend.V_dend;
    chPrms_prevComp1 = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + PREVSTATESTARTADD + DEND_H]); //&cellCompParamsPtr->prevCellState->dend.Hcurrent_q;
    chPrms_newComp1 = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + NEXTSTATESTARTADD + DEND_H]); //&cellCompParamsPtr->newCellState->dend.Hcurrent_q;
    //Compute
    DendHCurr(chPrms_v, chPrms_prevComp1, chPrms_newComp1);

    // //Prepare pointers to inputs/outputs
    chPrms_v = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + PREVSTATESTARTADD + DEND_V]);  //&cellCompParamsPtr->prevCellState->dend.V_dend;
    chPrms_prevComp1 = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + PREVSTATESTARTADD + DEND_CAL]); //&cellCompParamsPtr->prevCellState->dend.Calcium_r;
    chPrms_newComp1 = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + NEXTSTATESTARTADD + DEND_CAL]); //&cellCompParamsPtr->newCellState->dend.Calcium_r;
    // //Compute
    DendCaCurr(chPrms_v, chPrms_prevComp1, chPrms_newComp1);

    // //Prepare pointers to inputs/outputs
    chPrms_prevComp1 = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + PREVSTATESTARTADD + DEND_P]); //&cellCompParamsPtr->prevCellState->dend.Potassium_s;
    chPrms_prevComp2 = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + PREVSTATESTARTADD + DEND_CA2]); //&cellCompParamsPtr->prevCellState->dend.Ca2Plus;
    chPrms_newComp1 = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + NEXTSTATESTARTADD + DEND_P]);  //&cellCompParamsPtr->newCellState->dend.Potassium_s;
    //Compute
    DendKCurr(chPrms_prevComp1, chPrms_prevComp2, chPrms_newComp1);

    // //Prepare pointers to inputs/outputs
    chPrms_prevComp1 = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + PREVSTATESTARTADD + DEND_CA2]); //&cellCompParamsPtr->prevCellState->dend.Ca2Plus;
    chPrms_prevComp2 = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + PREVSTATESTARTADD + DEND_I]); //&cellCompParamsPtr->prevCellState->dend.I_CaH;
    chPrms_newComp1 = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + NEXTSTATESTARTADD + DEND_CA2]);  //&cellCompParamsPtr->newCellState->dend.Ca2Plus;
    //Compute
    DendCal(chPrms_prevComp1, chPrms_prevComp2, chPrms_newComp1);

	chComps_neighbours = &cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + VNEIGHSTARTADD];
	vDend = cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + PREVSTATESTARTADD + DEND_V];
    ...
    chComps_iApp = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + IAPP_IN]); //&cellCompParamsPtr->iAppIn;
    chComps_vDend = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + PREVSTATESTARTADD]); //&cellCompParamsPtr->prevCellState->dend.V_dend;
    chComps_newVDend = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + NEXTSTATESTARTADD]); //&cellCompParamsPtr->newCellState->dend.V_dend;
    chComps_vSoma = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + PREVSTATESTARTADD + SOMA_V]); //&cellCompParamsPtr->prevCellState->soma.V_soma
    chComps_q = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + NEXTSTATESTARTADD + DEND_H]); // &cellCompParamsPtr->newCellState->dend.Hcurrent_q;
    chComps_r = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + NEXTSTATESTARTADD + DEND_CAL]); //&cellCompParamsPtr->newCellState->dend.Calcium_r;
    chComps_s = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + NEXTSTATESTARTADD + DEND_P]); //&cellCompParamsPtr->newCellState->dend.Potassium_s;
    chComps_newI_CaH = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + NEXTSTATESTARTADD + DEND_I]); //&cellCompParamsPtr->newCellState->dend.I_CaH;
...
void CompSoma(global mod_prec *cellCompParamsPtr, int k, int j){
    ...
    global mod_prec *chPrms_prevComp1, *chPrms_prevComp2;
    global mod_prec *chPrms_newComp1, *chPrms_newComp2;
    global mod_prec *chComps_g_CaL;
    global mod_prec *chComps_vSoma;
    global mod_prec *chComps_vDend;
    global mod_prec *chComps_vAxon;
    global mod_prec *chComps_k, *chComps_l, *chComps_m, *chComps_h, *chComps_n, *chComps_x_s;
    global mod_prec *chComps_newVSoma;

    // update somatic components
    // SCHWEIGHOFER: was ist das?

    //Prepare pointers to inputs/outputs
    chPrms_v = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + PREVSTATESTARTADD + SOMA_V]); //&cellCompParamsPtr->prevCellState->soma.V_soma;
    chPrms_prevComp1 = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + PREVSTATESTARTADD + SOMA_CK]); //&cellCompParamsPtr->prevCellState->soma.Calcium_k;
    chPrms_prevComp2 = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + PREVSTATESTARTADD + SOMA_CL]); //&cellCompParamsPtr->prevCellState->soma.Calcium_l;
    chPrms_newComp1 = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + NEXTSTATESTARTADD + SOMA_CK]); //&cellCompParamsPtr->newCellState->soma.Calcium_k;
    chPrms_newComp2 = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + NEXTSTATESTARTADD + SOMA_CL]); //&cellCompParamsPtr->newCellState->soma.Calcium_l;
    //Compute
    SomaCalcium(chPrms_v, chPrms_prevComp1, chPrms_prevComp2, chPrms_newComp1, chPrms_newComp2);

    //Prepare pointers to inputs/outputs
    chPrms_v = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + PREVSTATESTARTADD + SOMA_V]); //&cellCompParamsPtr->prevCellState->soma.V_soma;
    chPrms_prevComp1 = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + PREVSTATESTARTADD + SOMA_SM]); //&cellCompParamsPtr->prevCellState->soma.Sodium_m;
    chPrms_prevComp2 = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + PREVSTATESTARTADD + SOMA_SH]); //&cellCompParamsPtr->prevCellState->soma.Sodium_h;
    chPrms_newComp1 = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + NEXTSTATESTARTADD + SOMA_SM]); //&cellCompParamsPtr->newCellState->soma.Sodium_m;
    chPrms_newComp2 = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + NEXTSTATESTARTADD + SOMA_SH]); //&cellCompParamsPtr->newCellState->soma.Sodium_h;
    //Compute
    SomaSodium(chPrms_v, chPrms_prevComp1, chPrms_prevComp2, chPrms_newComp1, chPrms_newComp2);

    // //Prepare pointers to inputs/outputs
    chPrms_v = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + PREVSTATESTARTADD + SOMA_V]); //&cellCompParamsPtr->prevCellState->soma.V_soma;
    chPrms_prevComp1 = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + PREVSTATESTARTADD + SOMA_PN]); //&cellCompParamsPtr->prevCellState->soma.Potassium_n;
    chPrms_prevComp2 = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + PREVSTATESTARTADD + SOMA_PP]); //&cellCompParamsPtr->prevCellState->soma.Potassium_p;
    chPrms_newComp1 = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + NEXTSTATESTARTADD + SOMA_PN]); //&cellCompParamsPtr->newCellState->soma.Potassium_n;
    chPrms_newComp2 = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + NEXTSTATESTARTADD + SOMA_PP]); //&cellCompParamsPtr->newCellState->soma.Potassium_p;
    //Compute
    SomaPotassium(chPrms_v, chPrms_prevComp1, chPrms_prevComp2, chPrms_newComp1, chPrms_newComp2);

    // //Prepare pointers to inputs/outputs
    chPrms_v = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + PREVSTATESTARTADD + SOMA_V]); //&cellCompParamsPtr->prevCellState->soma.V_soma;
    chPrms_prevComp1 =&(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + PREVSTATESTARTADD + SOMA_PXS]); //&cellCompParamsPtr->prevCellState->soma.Potassium_x_s;
    chPrms_newComp1 = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + NEXTSTATESTARTADD + SOMA_PXS]); //&cellCompParamsPtr->newCellState->soma.Potassium_x_s;
    //Compute
    SomaPotassiumX(chPrms_v, chPrms_prevComp1, chPrms_newComp1);

    chComps_g_CaL = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + PREVSTATESTARTADD + SOMA_G]); //&cellCompParamsPtr->prevCellState->soma.g_CaL;
    chComps_vDend = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + PREVSTATESTARTADD + DEND_V]); //&cellCompParamsPtr->prevCellState->dend.V_dend;
    chComps_vSoma = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + PREVSTATESTARTADD + SOMA_V]); //&cellCompParamsPtr->prevCellState->soma.V_soma;
    chComps_newVSoma = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + NEXTSTATESTARTADD + SOMA_V]); //&cellCompParamsPtr->newCellState->soma.V_soma;
    chComps_vAxon = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + PREVSTATESTARTADD + AXON_V]); //&cellCompParamsPtr->prevCellState->axon.V_axon;
    chComps_k = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + NEXTSTATESTARTADD + SOMA_CK]); //&cellCompParamsPtr->newCellState->soma.Calcium_k;
    chComps_l = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + NEXTSTATESTARTADD + SOMA_CL]); //&cellCompParamsPtr->newCellState->soma.Calcium_l;
    chComps_m = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + NEXTSTATESTARTADD + SOMA_SM]); //&cellCompParamsPtr->newCellState->soma.Sodium_m;
    chComps_h = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + NEXTSTATESTARTADD + SOMA_SH]); //&cellCompParamsPtr->newCellState->soma.Sodium_h;
    chComps_n = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + NEXTSTATESTARTADD + SOMA_PN]); //&cellCompParamsPtr->newCellState->soma.Potassium_n;
    chComps_x_s = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + NEXTSTATESTARTADD + SOMA_PXS]); // &cellCompParamsPtr->newCellState->soma.Potassium_x_s;
...
void CompAxon(global mod_prec *cellCompParamsPtr, int k, int j){
    ...
    global mod_prec *chComps_vSoma;
    global mod_prec *chComps_vAxon;
    global mod_prec *chComps_m_a, *chComps_h_a, *chComps_x_a;
    global mod_prec *chComps_newVAxon;

    // update somatic components
    // SCHWEIGHOFER:

    //Prepare pointers to inputs/outputs
    chPrms_v = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + PREVSTATESTARTADD + AXON_V]);//prevCellState->axon.V_axon;
    chPrms_prevComp1 = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + PREVSTATESTARTADD + AXON_SH]);//prevCellState->axon.Sodium_h_a;
    chPrms_newComp1 = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + NEXTSTATESTARTADD + AXON_SH]);//&cellCompParamsPtr->newCellState->axon.Sodium_h_a;
    chPrms_newComp2 = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + NEXTSTATESTARTADD + AXON_SM]);//&cellCompParamsPtr->newCellState->axon.Sodium_m_a;
    //Compute
    AxonSodium(chPrms_v, chPrms_prevComp1, chPrms_newComp1, chPrms_newComp2);

    //Prepare pointers to inputs/outputs
    chPrms_v = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + PREVSTATESTARTADD + AXON_V]);//&cellCompParamsPtr->prevCellState->axon.V_axon;
    chPrms_prevComp1 = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + PREVSTATESTARTADD + AXON_P]);//&cellCompParamsPtr->prevCellState->axon.Potassium_x_a;
    chPrms_newComp1 = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + NEXTSTATESTARTADD + AXON_P]);//&cellCompParamsPtr->newCellState->axon.Potassium_x_a;
    //Compute
    ...
    chComps_vSoma = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + PREVSTATESTARTADD + SOMA_V]);//&cellCompParamsPtr->prevCellState->soma.V_soma;
    chComps_vAxon = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + PREVSTATESTARTADD + AXON_V]);//&cellCompParamsPtr->prevCellState->axon.V_axon;
    chComps_newVAxon = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + NEXTSTATESTARTADD + AXON_V]);//&cellCompParamsPtr->newCellState->axon.V_axon;
    chComps_m_a = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + NEXTSTATESTARTADD + AXON_SM]);//&cellCompParamsPtr->newCellState->axon.Sodium_m_a;
    chComps_h_a = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + NEXTSTATESTARTADD + AXON_SH]);//&cellCompParamsPtr->newCellState->axon.Sodium_h_a;
    chComps_x_a = &(cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + NEXTSTATESTARTADD + AXON_P]);//&cellCompParamsPtr->newCellState->axon.Potassium_x_a;
...
**/
__kernel void compute_kernel(global mod_prec *cellStatePtr, global mod_prec *cellCompParamsPtr, global uint *i, global mod_prec *iApp){
    // initialize variables
    int j = get_global_id(0);		// dim 1
    int k = get_global_id(1);		// dim 2
	int n;							// loop counter
    
	// fill compute parameters
	cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + IAPP_IN] = *iApp;
	
	// copying to cellcompparams
	for(n=0; n<CELL_STATE_SIZE; n++){
		cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + PREVSTATESTARTADD + n] = cellStatePtr[CELLSTATE_BASEADDR_OPENCL(((*i) % 2),j,k) + n];
		cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + NEXTSTATESTARTADD + n] = cellStatePtr[CELLSTATE_BASEADDR_OPENCL((((*i) % 2)^1),j,k) + n];
	}

	// compute all
	CompDend(cellCompParamsPtr, k, j); 
    CompSoma(cellCompParamsPtr, k, j);
    CompAxon(cellCompParamsPtr, k, j);
	
	// copying back to states
	for(n=0; n<CELL_STATE_SIZE; n++){
		cellStatePtr[CELLSTATE_BASEADDR_OPENCL((((*i) % 2)^1),j,k) + n] = cellCompParamsPtr[CELLCOMP_BASEADDR_OPENCL(j,k) + NEXTSTATESTARTADD + n];
	}
}