...
int main(int argc, char *argv[]){
    ...
    if(argc < 4){
        printf("Usage: %s (matrix/vector_size start) (matrix/vector_size end) (local group size)\n", argv[0]);
        return 0;
    }
	...
	// check values
    if((size_start == 0) || (size_end == 0) || (localSize == 0)){
	...
	cl_double *vector = (double *)malloc(sizeof(cl_double)*size_end);
	cl_double *matrix = (double *)malloc(sizeof(cl_double)*size_end*size_end);
	cl_double *result_sq = (double *)malloc(sizeof(cl_double)*size_end);
	cl_double *result_pl = (double *)malloc(sizeof(cl_double)*size_end);
	...
	} // end for(size = size_start; size <= size_end; size += localSize)
